...
private Logger	logger=LoggerFactory.getLogger(LoanDisbursement.class);
	...
	public LoanDisbursement insertData(CustomerDetails customer) {
		...
		String content2="as per your vehicle quatation provided to"+customer.getCustomerFirstName()+" "
						+customer.getCustomerLastName()+"And further application aproved loan amount "+customer.getCustomerSanctionLetter().getLoanAmountSanctioned()+
						"is sanctiond by shree ganeh finace and amount is tranfer to your account"+customer.getCustomerDealer().getDealerBankDetails();
		...
		try {
			...
			mimemessageHelper.setText("Dear Dealer, We have recived sanction letter & happy to let you that we will get sanction amount");
